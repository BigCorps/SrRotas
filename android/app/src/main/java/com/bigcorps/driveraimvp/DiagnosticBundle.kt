...
object DiagnosticBundle {
    fun build(context:Context):String{
        val repo=SettingsRepository(context);val s=repo.load();val store=LocalStore.get(context);val summary=JourneyCoordinator.currentSummary(context)
        return JSONObject().apply{
            ...
            put("hud",JSONObject().apply{put("order",s.hudMetricOrder);put("enabled",s.hudEnabledMetrics);put("position",s.hudPosition);put("theme",s.hudTheme);put("opacity",s.hudOpacity);put("font_size",s.hudFontSize);put("color_blind",s.colorBlindMode)})
            ...
            put("privacy_note","Compartilhamento explícito. Nenhuma screenshot, device_token ou código de pareamento é incluído. O texto OCR bruto pode conter conteúdo visível na tela.")
        }.toString(2)
    }
    fun share(context:Context){val send=Intent(Intent.ACTION_SEND).apply{type="text/plain";putExtra(Intent.EXTRA_SUBJECT,"Diagnóstico Sr. Rotas ${BuildConfig.VERSION_NAME}");putExtra(Intent.EXTRA_TEXT,build(context))};val chooser=Intent.createChooser(send,"Compartilhar diagnóstico do Sr. Rotas");if(context !is Activity)chooser.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);context.startActivity(chooser)}
