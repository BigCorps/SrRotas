...
class MediaProjectionOcrService : Service() {
    ...
    // Preserva o primeiro frame novo e também o mais recente.
    ...
    private fun startProjectionFromIntent(intent: Intent) {
        ...
        if (resultCode != Activity.RESULT_OK) {
            LocalLog.append(
                this,
                "MediaProjection não autorizado: resultCode=$resultCode",
            )
            stopSelf()
            return
        }
        ...
        reader.setOnImageAvailableListener(
            { availableReader ->
                onImage(availableReader, width, height)
            },
            handler,
        )
        ...
        LocalLog.append(
            this,
            "Jornada MediaProjection iniciada: ${width}x${height} @ ${density}dpi",
        )
        sendBroadcast(
            Intent(AppSignals.ACTION_CAPTURE_UPDATED).setPackage(packageName),
        )
    }
    ...
    private fun prepareForOcr(source: Bitmap): Bitmap {
        ...
        if (scaled !== source) {
            if (!scaleLogged) {
                ...
                LocalLog.append(
                    this,
                    "OCR otimizado: ${source.width}x${source.height} -> " +
    ...
    private fun processBitmap(bitmap: Bitmap) {
        ...
        recognizer.process(InputImage.fromBitmap(bitmap, 0))
            .addOnSuccessListener { result ->
                ...
                val offers =
                    if (gate == UberScreenGate.Kind.OFFER_CANDIDATE) {
                ...
                if (offers.isNotEmpty()) {
                    ...
                    performance.dispatchCompleted(
                        SystemClock.elapsedRealtime() - dispatchStarted,
                    )

                    if (settings.privateScreenshotEnabled) {
                        offers.maxByOrNull { it.confidence }?.let {
                            PrivateScreenshotStore.save(this, bitmap, it)
                        }
                    }
                } else {
                    when (gate) {
                        UberScreenGate.Kind.OFFER_CANDIDATE ->
                            saveCandidateDiagnosticOnce(
                                result.text,
            ...
            .addOnFailureListener {
                LocalLog.append(
                    this,
                    "OCR MediaProjection falhou: ${it.message}",
                )
            }
    ...
    private fun saveCandidateDiagnosticOnce(
        raw: String,
        method: String,
    ) {
        ...
        if (
            fp == lastRawFingerprint ||
            now - lastCandidateDiagnosticAt <
            CANDIDATE_DIAGNOSTIC_INTERVAL_MS
        ) {
            return
        }
    ...
    private fun logRejectedFrame(
        ...
        LocalLog.append(
            this,
            "FRAME ignorado · $label · $chars caracteres",
        )
    }
