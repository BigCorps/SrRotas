...
class SettingsRepository(context: Context) {
    ...
    fun save(s: DriverSettings) {
        prefs.edit()
            ...
            .putString("hud_theme", normalizeTheme(s.hudTheme)).putString("hud_card_size", normalizeSize(s.hudCardSize))
    ...
    private fun migrateV05HudDefaults() {
        ...
        if ((prefs.getString("hud_metric_order", oldOrder) ?: oldOrder) == oldOrder) edit.putString("hud_metric_order", DriverSettings().hudMetricOrder)
        if ((prefs.getString("hud_enabled_metrics", oldEnabled) ?: oldEnabled) == oldEnabled) edit.putString("hud_enabled_metrics", DriverSettings().hudEnabledMetrics)
    ...
    private fun migrateV06VisualDefaults() {
        ...
        val size = when { font <= 15 -> "compact"; font >= 19 -> "large"; else -> "normal" }
    ...
    private fun normalizeSize(v: String) = when (v) { "compact", "normal", "large" -> v; else -> "normal" }
