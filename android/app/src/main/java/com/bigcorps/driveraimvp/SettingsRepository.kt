...
class SettingsRepository(context: Context) {
    ...
    private val prefs = appContext.getSharedPreferences("driver_ai_settings", Context.MODE_PRIVATE)
    ...
    fun load(): DriverSettings = DriverSettings(
        ...
        minPerKm = double("min_per_km", 1.80),
        redPerKmBelow = double("red_per_km_below", 1.45),
        minPerHour = double("min_per_hour", 35.0),
        redPerHourBelow = double("red_per_hour_below", 28.0),
        goodRatingFrom = double("good_rating_from", 4.85),
        redRatingBelow = double("red_rating_below", 4.70),
        minPerMinute = double("min_per_minute", 0.60),
        redPerMinuteBelow = double("red_per_minute_below", 0.48),
        minFare = double("min_fare", 0.0),
        maxPickupKm = double("max_pickup_km", 5.0),
        minProfit = double("min_profit", 0.0),
        minProfitPerHour = double("min_profit_per_hour", 0.0),
        redProfitPerHourBelow = double("red_profit_per_hour_below", 0.0),
        minProfitPercent = double("min_profit_percent", 0.0),
        redProfitPercentBelow = double("red_profit_percent_below", 0.0),
        ...
        hudPosition = prefs.getString("hud_position", "left") ?: "left",
        hudTheme = prefs.getString("hud_theme", "auto") ?: "auto",
        hudCardSize = prefs.getString("hud_card_size", "normal") ?: "normal",
        hudDismissOnTap = prefs.getBoolean("hud_dismiss_on_tap", true),
        hudDragEnabled = prefs.getBoolean("hud_drag_enabled", true),
        colorBlindMode = prefs.getBoolean("color_blind_mode", false),
        hudOpacity = prefs.getInt("hud_opacity", 90).coerceIn(30, 100),
        ...
        collectiveStatsOptIn = prefs.getBoolean("collective_stats_opt_in", false),
    ...
    fun save(s: DriverSettings) {
        prefs.edit()
            ...
            .putString("min_per_km", s.minPerKm.toString())
            .putString("red_per_km_below", s.redPerKmBelow.toString())
            .putString("min_per_hour", s.minPerHour.toString())
            .putString("red_per_hour_below", s.redPerHourBelow.toString())
            .putString("good_rating_from", s.goodRatingFrom.toString())
            .putString("red_rating_below", s.redRatingBelow.toString())
            .putString("min_per_minute", s.minPerMinute.toString())
            .putString("red_per_minute_below", s.redPerMinuteBelow.toString())
            .putString("min_fare", s.minFare.toString())
            .putString("max_pickup_km", s.maxPickupKm.toString())
            .putString("min_profit", s.minProfit.toString())
            .putString("min_profit_per_hour", s.minProfitPerHour.toString())
            .putString("red_profit_per_hour_below", s.redProfitPerHourBelow.toString())
            .putString("min_profit_percent", s.minProfitPercent.toString())
            .putString("red_profit_percent_below", s.redProfitPercentBelow.toString())
            .putString("cost_per_km", s.costPerKm.toString())
            .putBoolean("ocr_enabled", s.ocrEnabled)
            .putBoolean("consent_accepted", s.consentAccepted)
            .putString("hud_metric_order", s.hudMetricOrder)
            .putString("hud_enabled_metrics", s.hudEnabledMetrics)
            .putString("hud_position", s.hudPosition)
            .putString("hud_theme", normalizeTheme(s.hudTheme))
            .putString("hud_card_size", normalizeSize(s.hudCardSize))
            .putBoolean("hud_dismiss_on_tap", s.hudDismissOnTap)
            .putBoolean("hud_drag_enabled", s.hudDragEnabled)
            .putBoolean("color_blind_mode", s.colorBlindMode)
            .putInt("hud_opacity", s.hudOpacity.coerceIn(30, 100))
            .putInt("hud_font_size", s.hudFontSize.coerceIn(14, 24))
            .putBoolean("text_notification_enabled", s.textNotificationEnabled)
            ...
            .putString(
                ...
                HudPresentation.normalizedVoiceOrder(s.voiceMetricOrder)
                    .filter { it in s.voiceEnabledMetrics.split(',').map(String::trim).toSet() }
                    .joinToString(","),
            )
            .putBoolean("private_screenshot_enabled", s.privateScreenshotEnabled)
            .putBoolean("collective_stats_opt_in", s.collectiveStatsOptIn)
            .putString("default_passenger_message", s.defaultPassengerMessage.take(600))
            .apply()
    }
    ...
    fun markOnboardingStep(step: Int) {
        prefs.edit().putInt("onboarding_step", step.coerceIn(0, 5)).apply()
    }

    fun completeOnboarding() {
        prefs.edit().putBoolean("onboarding_completed", true).putInt("onboarding_step", 5).apply()
    }

    fun restartOnboarding() {
        prefs.edit().putBoolean("onboarding_completed", false).putInt("onboarding_step", 0).apply()
    }

    fun saveHudPosition(x: Int, y: Int) {
        prefs.edit()
            .putInt("hud_custom_x", x)
            .putInt("hud_custom_y", y)
            .putBoolean("hud_custom_position", true)
            .apply()
    }
    ...
    fun resetHudPosition() {
        prefs.edit()
            .remove("hud_custom_x")
            .remove("hud_custom_y")
            .putBoolean("hud_custom_position", false)
            .apply()
    }
    ...
    private fun migrateV06VisualDefaults() {
        if (prefs.getBoolean("hud_v06_migrated", false)) return
        val font = prefs.getInt("hud_font_size", 16)
        val size = when {
            font <= 15 -> "compact"
            font >= 19 -> "large"
            else -> "normal"
        }
        val oldTheme = prefs.getString("hud_theme", "light") ?: "light"
        val migratedTheme = if (oldTheme == "green") "dark" else oldTheme
        prefs.edit()
            .putString("hud_card_size", prefs.getString("hud_card_size", size) ?: size)
            .putString("hud_theme", normalizeTheme(migratedTheme))
    ...
    private fun migrateV12CanonicalDomain() {
        ...
        val current = prefs.getString("backend_url", DEFAULT_BACKEND_URL)?.trim()?.trimEnd('/')
            ?: DEFAULT_BACKEND_URL
        ...
        prefs.edit()
            .putString("backend_url", migrated)
            .putBoolean("domain_v12_migrated", true)
            .apply()
    }
    ...
    fun clearAllUserData() {
        runCatching { HistoricalImportStore.get(appContext).clearAll() }
        prefs.edit().clear().apply()
    }

    private fun normalizeTheme(v: String) = when (v) {
        "light", "dark", "auto" -> v
        else -> "auto"
    }

    private fun normalizeSize(v: String) = when (v) {
        "compact", "normal", "large" -> v
        else -> "normal"
    }

    private fun double(key: String, fallback: Double) =
        prefs.getString(key, fallback.toString())?.toDoubleOrNull() ?: fallback
    ...
    fun latestSummary(): String =
        prefs.getString("latest_summary", "Nenhuma oferta reconhecida ainda.")
    ...
    fun setCurrentJourney(id: String, startedAt: String) {
        prefs.edit()
            .putString("current_journey_id", id)
            .putString("current_journey_started_at", startedAt)
            .apply()
    }
    ...
    fun clearCurrentJourney() {
        prefs.edit().remove("current_journey_id").remove("current_journey_started_at").apply()
    }
}
