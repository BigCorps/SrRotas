...
class LocalStore private constructor(context: Context) : SQLiteOpenHelper(context.applicationContext, DB_NAME, null, DB_VERSION) {
    ...
    override fun onUpgrade(db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
        ...
        if (oldVersion < 4) createV15Tables(db)
        createIndexes(db)
    }
    ...
    private fun createIndexes(db: SQLiteDatabase) {
        ...
        db.execSQL("create index if not exists local_journey_events_journey_idx on local_journey_events(journey_id, occurred_at desc)")
        db.execSQL("create index if not exists local_journey_events_sync_idx on local_journey_events(sync_state, created_at_ms)")
        db.execSQL("create index if not exists local_ride_outcomes_journey_idx on local_ride_outcomes(journey_id, status, updated_at_ms desc)")
        db.execSQL("create index if not exists local_ride_outcomes_sync_idx on local_ride_outcomes(sync_state, updated_at_ms)")
        db.execSQL("create index if not exists local_zone_exposure_journey_idx on local_zone_exposure(journey_id, started_at desc)")
        db.execSQL("create index if not exists local_zone_exposure_cell_idx on local_zone_exposure(cell, started_at desc)")
        db.execSQL("create index if not exists local_zone_exposure_sync_idx on local_zone_exposure(sync_state, created_at_ms)")
    }
    ...
    @Synchronized fun endJourney(id: String, reason: String): JourneySummary? {
        ...
        writableDatabase.update("local_journeys", ContentValues().apply { put("ended_at", endedAt); put("end_reason", reason.take(120)) }, "id = ?", arrayOf(id))
    ...
    @Synchronized fun saveOrUpdateContext(localId: String, context: OfferContext, syncState: Int = 0) {
        writableDatabase.insertWithOnConflict("local_offer_context", null, ContentValues().apply {
            put("local_offer_id", localId)
            putNullable("pickup_label", context.pickupLabel); putNullable("destination_label", context.destinationLabel)
            putNullable("pickup_lat", context.pickupLat); putNullable("pickup_lng", context.pickupLng); putNullable("destination_lat", context.destinationLat); putNullable("destination_lng", context.destinationLng)
            putNullable("pickup_cell", context.pickupCell); putNullable("destination_cell", context.destinationCell); putNullable("estimated_arrival_at", context.estimatedArrivalAt)
            put("context_confidence", context.contextConfidence.coerceIn(0.0, 1.0)); put("geocode_status", context.geocodeStatus); putNullable("geocode_source", context.geocodeSource)
            put("context_version", context.contextVersion); put("source_type", context.sourceType); put("time_source", context.timeSource); put("sync_state", syncState); put("updated_at_ms", System.currentTimeMillis())
    ...
    fun pendingOfferContexts(limit: Int = 50): List<PendingOfferContext> {
        ...
        readableDatabase.rawQuery(sql, arrayOf(limit.coerceIn(1, 200).toString())).use { c ->
            while (c.moveToNext()) rows += PendingOfferContext(c.getString(c.getColumnIndexOrThrow("local_offer_id")), c.getString(c.getColumnIndexOrThrow("dedupe_key")), c.toOfferContext())
        }
        return rows
    }

    fun contextForOffer(localId: String): OfferContext? = readableDatabase.query("local_offer_context", CONTEXT_COLUMNS, "local_offer_id = ?", arrayOf(localId), null, null, null, "1").use { if (!it.moveToFirst()) null else it.toOfferContext() }

    fun cachedGeocode(normalizedQuery: String): CachedGeocode? = readableDatabase.query("local_geocode_cache", arrayOf("lat", "lng", "cell"), "normalized_query = ?", arrayOf(normalizedQuery), null, null, null, "1").use {
        if (!it.moveToFirst()) null else CachedGeocode(it.getDouble(0), it.getDouble(1), if (it.isNull(2)) null else it.getString(2))
    }

    @Synchronized fun cacheGeocode(normalizedQuery: String, originalLabel: String, lat: Double, lng: Double, cell: String?) {
        writableDatabase.insertWithOnConflict("local_geocode_cache", null, ContentValues().apply {
            put("normalized_query", normalizedQuery); put("original_label", originalLabel.take(220)); put("lat", lat); put("lng", lng); putNullable("cell", cell); put("updated_at_ms", System.currentTimeMillis())
        }, SQLiteDatabase.CONFLICT_REPLACE)
    }

    // ---- 0.15: jornada operacional ------------------------------------------------------------

    @Synchronized fun recordJourneyEvent(journeyId: String, eventType: String, state: JourneyOperationalState, occurredAt: String = Instant.now().toString()): JourneyStateEvent {
        val event = JourneyStateEvent(journeyId = journeyId, eventType = eventType.lowercase().take(30), state = state, occurredAt = occurredAt)
        writableDatabase.insertOrThrow("local_journey_events", null, ContentValues().apply {
            put("id", event.id); put("journey_id", event.journeyId); put("event_type", event.eventType); put("state", event.state.name); put("occurred_at", event.occurredAt); put("sync_state", 0); put("created_at_ms", System.currentTimeMillis())
        })
        return event
    }

    fun currentJourneyState(journeyId: String): JourneyOperationalState {
        readableDatabase.query("local_journey_events", arrayOf("state"), "journey_id = ?", arrayOf(journeyId), null, null, "occurred_at desc, created_at_ms desc", "1").use { c ->
            if (c.moveToFirst()) return runCatching { JourneyOperationalState.valueOf(c.getString(0)) }.getOrDefault(JourneyOperationalState.ACTIVE)
        }
        return journey(journeyId)?.let { if (it.endedAt == null) JourneyOperationalState.ACTIVE else JourneyOperationalState.ENDED } ?: JourneyOperationalState.NOT_STARTED
    }

    fun pendingJourneyEvents(limit: Int = 50): List<JourneyStateEvent> {
        val rows = mutableListOf<JourneyStateEvent>()
        readableDatabase.query("local_journey_events", arrayOf("id", "journey_id", "event_type", "state", "occurred_at"), "sync_state = 0", null, null, null, "created_at_ms asc", limit.coerceIn(1, 200).toString()).use { c ->
            while (c.moveToNext()) rows += JourneyStateEvent(c.getString(0), c.getString(1), c.getString(2), runCatching { JourneyOperationalState.valueOf(c.getString(3)) }.getOrDefault(JourneyOperationalState.ACTIVE), c.getString(4))
        }
        return rows
    }

    @Synchronized fun markJourneyEventSynced(id: String) { writableDatabase.update("local_journey_events", ContentValues().apply { put("sync_state", 1) }, "id = ?", arrayOf(id)) }

    @Synchronized fun ensureOfferedOutcome(offer: RideOffer, source: String = "offer_engine"): RideOutcome? {
        val journeyId = offer.journeyId?.takeIf { it.isNotBlank() } ?: return null
        val existing = rideOutcomeForOffer(offer.localId)
        if (existing != null) return existing
        val outcome = RideOutcome(localOfferId = offer.localId, journeyId = journeyId, status = RideOperationalStatus.OFFERED, source = source, revision = 1)
        saveRideOutcome(outcome)
        return outcome
    }

    @Synchronized fun updateRideOutcome(localOfferId: String, journeyId: String, status: RideOperationalStatus, source: String, occurredAt: String = Instant.now().toString()): RideOutcome {
        val previous = rideOutcomeForOffer(localOfferId)
        val revision = (previous?.revision ?: 0) + 1
        val startedAt = when (status) {
            RideOperationalStatus.DOING_RIDE -> previous?.startedAt ?: occurredAt
            else -> previous?.startedAt
        }
        val completedAt = if (status == RideOperationalStatus.COMPLETED) occurredAt else null
        val cancelledAt = if (status == RideOperationalStatus.CANCELLED || status == RideOperationalStatus.NOT_COMPLETED) occurredAt else null
        val correctedAt = if (previous != null || source == "history") occurredAt else null
        val outcome = RideOutcome(localOfferId, journeyId, status, startedAt, completedAt, cancelledAt, correctedAt, source.take(40), revision)
        saveRideOutcome(outcome)
        return outcome
    }

    private fun saveRideOutcome(outcome: RideOutcome) {
        writableDatabase.insertWithOnConflict("local_ride_outcomes", null, ContentValues().apply {
            put("local_offer_id", outcome.localOfferId); put("journey_id", outcome.journeyId); put("status", outcome.status.name)
            putNullable("started_at", outcome.startedAt); putNullable("completed_at", outcome.completedAt); putNullable("cancelled_at", outcome.cancelledAt); putNullable("corrected_at", outcome.correctedAt)
            put("source", outcome.source); put("revision", outcome.revision); put("sync_state", 0); put("updated_at_ms", System.currentTimeMillis())
        }, SQLiteDatabase.CONFLICT_REPLACE)
    }

    fun rideOutcomeForOffer(localOfferId: String): RideOutcome? = readableDatabase.query("local_ride_outcomes", RIDE_OUTCOME_COLUMNS, "local_offer_id = ?", arrayOf(localOfferId), null, null, null, "1").use { if (!it.moveToFirst()) null else it.toRideOutcome() }

    fun currentDoingRide(journeyId: String): RideOutcome? = readableDatabase.query("local_ride_outcomes", RIDE_OUTCOME_COLUMNS, "journey_id = ? and status = ?", arrayOf(journeyId, RideOperationalStatus.DOING_RIDE.name), null, null, "updated_at_ms desc", "1").use { if (!it.moveToFirst()) null else it.toRideOutcome() }

    fun pendingRideOutcomes(limit: Int = 80): List<RideOutcome> {
        val rows = mutableListOf<RideOutcome>()
        readableDatabase.query("local_ride_outcomes", RIDE_OUTCOME_COLUMNS, "sync_state = 0", null, null, null, "updated_at_ms asc", limit.coerceIn(1, 250).toString()).use { c -> while (c.moveToNext()) rows += c.toRideOutcome() }
        return rows
    }

    @Synchronized fun markRideOutcomeSynced(localOfferId: String, revision: Int) {
        writableDatabase.update("local_ride_outcomes", ContentValues().apply { put("sync_state", 1) }, "local_offer_id = ? and revision = ?", arrayOf(localOfferId, revision.toString()))
    }

    @Synchronized fun openExposure(journeyId: String, cell: String, accuracyM: Double?, startedAt: String = Instant.now().toString()): RegionalExposure {
        currentOpenExposure(journeyId)?.let { return it }
        val exposure = RegionalExposure(journeyId = journeyId, cell = cell, startedAt = startedAt, locationAccuracyM = accuracyM)
        writableDatabase.insertOrThrow("local_zone_exposure", null, ContentValues().apply {
            put("id", exposure.id); put("journey_id", exposure.journeyId); put("cell", exposure.cell); put("started_at", exposure.startedAt); putNullable("location_accuracy_m", exposure.locationAccuracyM); put("sync_state", 0); put("created_at_ms", System.currentTimeMillis())
        })
        return exposure
    }

    fun currentOpenExposure(journeyId: String): RegionalExposure? = readableDatabase.query("local_zone_exposure", EXPOSURE_COLUMNS, "journey_id = ? and ended_at is null", arrayOf(journeyId), null, null, "created_at_ms desc", "1").use { if (!it.moveToFirst()) null else it.toExposure() }

    @Synchronized fun closeExposure(journeyId: String, reason: String, nextOfferLocalId: String? = null, endedAt: String = Instant.now().toString()): RegionalExposure? {
        val open = currentOpenExposure(journeyId) ?: return null
        val seconds = runCatching { Duration.between(Instant.parse(open.startedAt), Instant.parse(endedAt)).seconds.coerceAtLeast(0) }.getOrDefault(0)
        writableDatabase.update("local_zone_exposure", ContentValues().apply {
            put("ended_at", endedAt); put("duration_seconds", seconds); put("close_reason", reason.take(40)); putNullable("next_offer_local_id", nextOfferLocalId); put("sync_state", 0)
        }, "id = ?", arrayOf(open.id))
        return open.copy(endedAt = endedAt, durationSeconds = seconds, closeReason = reason.take(40), nextOfferLocalId = nextOfferLocalId)
    }

    fun pendingExposures(limit: Int = 80): List<RegionalExposure> {
        val rows = mutableListOf<RegionalExposure>()
        readableDatabase.query("local_zone_exposure", EXPOSURE_COLUMNS, "sync_state = 0 and ended_at is not null", null, null, null, "created_at_ms asc", limit.coerceIn(1, 250).toString()).use { c -> while (c.moveToNext()) rows += c.toExposure() }
        return rows
    }

    @Synchronized fun markExposureSynced(id: String) { writableDatabase.update("local_zone_exposure", ContentValues().apply { put("sync_state", 1) }, "id = ?", arrayOf(id)) }
    ...
    fun offersInRange(fromIso: String, toIso: String, limit: Int = 2000): List<RideOffer> = queryOffers("observed_at >= ? and observed_at < ?", arrayOf(fromIso, toIso), "observed_at asc", limit)
    fun journeysInRange(fromIso: String, toIso: String, limit: Int = 100): List<JourneyRecord> = queryJourneys("started_at >= ? and started_at < ?", arrayOf(fromIso, toIso), "started_at desc", limit)
    ...
    private fun Cursor.toOfferContext() = OfferContext(
        pickupLabel = stringOrNull("pickup_label"), destinationLabel = stringOrNull("destination_label"), pickupLat = doubleOrNull("pickup_lat"), pickupLng = doubleOrNull("pickup_lng"), destinationLat = doubleOrNull("destination_lat"), destinationLng = doubleOrNull("destination_lng"), pickupCell = stringOrNull("pickup_cell"), destinationCell = stringOrNull("destination_cell"), estimatedArrivalAt = stringOrNull("estimated_arrival_at"), contextConfidence = doubleOrNull("context_confidence") ?: 0.0, geocodeStatus = stringOrNull("geocode_status") ?: "unresolved", geocodeSource = stringOrNull("geocode_source"), contextVersion = stringOrNull("context_version") ?: OfferContextEngine.VERSION, sourceType = stringOrNull("source_type") ?: "live_ocr", timeSource = stringOrNull("time_source") ?: "system_observed_at"
    )

    private fun Cursor.toRideOutcome() = RideOutcome(
        localOfferId = getString(getColumnIndexOrThrow("local_offer_id")), journeyId = getString(getColumnIndexOrThrow("journey_id")), status = runCatching { RideOperationalStatus.valueOf(getString(getColumnIndexOrThrow("status"))) }.getOrDefault(RideOperationalStatus.OFFERED), startedAt = stringOrNull("started_at"), completedAt = stringOrNull("completed_at"), cancelledAt = stringOrNull("cancelled_at"), correctedAt = stringOrNull("corrected_at"), source = getString(getColumnIndexOrThrow("source")), revision = getInt(getColumnIndexOrThrow("revision"))
    )

    private fun Cursor.toExposure() = RegionalExposure(
        id = getString(getColumnIndexOrThrow("id")), journeyId = getString(getColumnIndexOrThrow("journey_id")), cell = getString(getColumnIndexOrThrow("cell")), startedAt = getString(getColumnIndexOrThrow("started_at")), endedAt = stringOrNull("ended_at"), durationSeconds = longOrNull("duration_seconds"), closeReason = stringOrNull("close_reason"), nextOfferLocalId = stringOrNull("next_offer_local_id"), locationAccuracyM = doubleOrNull("location_accuracy_m")
    )
    ...
    private fun Cursor.longOrNull(c: String): Long? { val i = getColumnIndexOrThrow(c); return if (isNull(i)) null else getLong(i) }
    ...
    private val RIDE_OUTCOME_COLUMNS = arrayOf("local_offer_id", "journey_id", "status", "started_at", "completed_at", "cancelled_at", "corrected_at", "source", "revision")
    private val EXPOSURE_COLUMNS = arrayOf("id", "journey_id", "cell", "started_at", "ended_at", "duration_seconds", "close_reason", "next_offer_local_id", "location_accuracy_m")
}
