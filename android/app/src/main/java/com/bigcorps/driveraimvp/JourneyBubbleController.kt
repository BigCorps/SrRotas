...
object JourneyBubbleController {
    ...
    /** Atualização completa, usada apenas em mudança de estado/configuração. */
    ...
    /**
     * 0.16: uma nova oferta não precisa recalcular tamanho/opacidade/layout do
     * mascote. Se o painel estiver fechado, não há trabalho visual a fazer.
     */
    ...
    private fun create(context: Context) {
        ...
        val icon = ImageView(context).apply {
            // Header/mascote continuam usando o logo original transparente.
        ...
        container.addView(
            ...
            LinearLayout.LayoutParams(
                UiKit.dp(context, 286),
                LinearLayout.LayoutParams.WRAP_CONTENT,
            ).apply { topMargin = UiKit.dp(context, 6) },
        )
    ...
    private fun rebuildPanel(context: Context) {
        ...
        latest?.let { offer ->
            val verdict = when (offer.verdict) {
                "boa" -> "BOA"
                "ruim" -> "ABAIXO"
                else -> "ATENÇÃO"
            }
            val metrics = buildString {
                ...
                offer.context?.destinationLabel
                    ?.takeIf { it.isNotBlank() }
                    ?.let { append("\n→ ${it.take(72)}") }
            }
            holder.addView(
                UiKit.margin(UiKit.body(context, metrics, 12f), top = 5),
            )
        }
        ...
        when {
            snapshot.isDoingRide -> {
                ...
                holder.addView(
                    UiKit.margin(
                        UiKit.primaryButton(context, "Finalizar corrida") {
                            JourneyCoordinator.completeCurrentRide(context, "bubble")
                        },
                        top = 7,
                    ),
                )
                holder.addView(
                    UiKit.margin(
                        UiKit.secondaryButton(context, "Cancelar / não concluída") {
                            JourneyCoordinator.cancelCurrentRide(context, "bubble")
                        },
                        top = 6,
                    ),
                )
            ...
            snapshot.journeyState == JourneyOperationalState.PAUSED -> {
                holder.addView(
                    UiKit.margin(
                        UiKit.primaryButton(context, "Retomar jornada") {
                ...
                holder.addView(
                    UiKit.margin(
                        UiKit.secondaryButton(context, "Encerrar jornada") {
                            JourneyCoordinator.endJourney(context, "bubble_end")
                        },
                        top = 6,
                    ),
                )
            ...
            snapshot.journeyState == JourneyOperationalState.ACTIVE -> {
                ...
                if (latest != null && latest.journeyId == snapshot.journeyId) {
                    holder.addView(
                        UiKit.margin(
                            UiKit.primaryButton(context, "Estou fazendo") {
                ...
                holder.addView(
                    UiKit.margin(
                        UiKit.secondaryButton(context, "Pausar jornada") {
                ...
                UiKit.body(context, "Abra o Sr. Rotas para iniciar uma jornada."),
            )
        }

        val prefs = JourneyUiPreferences(context)
        val configRow = LinearLayout(context).apply {
            orientation = LinearLayout.HORIZONTAL
            gravity = Gravity.CENTER_VERTICAL
        }

        configRow.addView(
            smallAction(context, "Tamanho ${prefs.sizeDp()}") {
                prefs.cycleSize()
                applyBubbleStyle(context)
        ...
        configRow.addView(
            smallAction(context, "Opacidade ${prefs.opacityPercent()}%") {
                prefs.cycleOpacity()
                applyBubbleStyle(context)
                rebuildPanel(context)
            },
    ...
    private fun mapsButton(
        ...
        val label =
            if (destination) "Abrir destino no Maps" else "Abrir retirada no Maps"

        holder.addView(
            UiKit.margin(
                UiKit.secondaryButton(context, label) {
    ...
    private fun historyButton(context: Context, holder: LinearLayout) {
        holder.addView(
            UiKit.margin(
                UiKit.secondaryButton(context, "Histórico") {
                    context.startActivity(
                        Intent(context, HistoryQuickActivity::class.java)
