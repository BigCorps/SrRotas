...
/** Histórico de ofertas com estado permanente da corrida realizada. */
class RideHistoryPanel027035(context: Context) : ScrollView(context) {
    ...
    init {
        ...
        body.addView(
            UiKit.margin(
                UiKit.secondaryButton(context, "Carregar mais ofertas") {
        ...
        root.addView(
            body,
            LinearLayout.LayoutParams(SrUi023.maxContentWidthPx(context), LinearLayout.LayoutParams.WRAP_CONTENT),
        )
    ...
    fun refresh() {
        ...
        offers.forEachIndexed { index, offer ->
            host.addView(UiKit.margin(card(offer), top = if (index == 0) 0 else 7))
        }
    }
    ...
    private fun card(o: RideOffer) = LinearLayout(context).apply {
        ...
        top.addView(
            SrUi023.title(context, "${platform(o)} · ${service(o)}", 12.5f),
            LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f),
        )
        if (completed) {
            top.addView(
                SrUi023.pill(context, "✓ REALIZADA", "good"),
                LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.WRAP_CONTENT).apply {
                    marginEnd = UiKit.dp(context, 7)
                },
            )
        }
        ...
        val pickup = o.context?.pickupLabel?.takeIf(String::isNotBlank)
            ?: o.context?.pickupCell?.takeIf(String::isNotBlank) ?: "—"
        val dest = o.context?.destinationLabel?.takeIf(String::isNotBlank)
            ?: o.context?.destinationCell?.takeIf(String::isNotBlank) ?: "—"
        ...
        actionRow.addView(
            smallButton(
                if (completed) "✓ Corrida realizada" else "Fiz essa corrida",
                UiKit.palette(context).good,
                filled = completed,
            ) {
                if (!completed) {
                    JourneyCoordinator.correctRide(context, o.localId, RideOperationalStatus.COMPLETED)
                    JourneyBubbleController.refresh(context)
                    Toast.makeText(context, "Corrida marcada como realizada.", Toast.LENGTH_SHORT).show()
                    post { refresh() }
                }
            },
            LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f),
    ...
    private fun metricRow(o: RideOffer) = LinearLayout(context).apply {
        orientation = LinearLayout.HORIZONTAL
        listOf(
            "R$/km" to value(o.perKm),
            "R$/min" to value(o.perMinute),
            "R$/h" to value(o.perHour),
            ...
            addView(
                ...
                LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f).apply {
                    if (i > 0) marginStart = UiKit.dp(context, 3)
                },
            )
        }
    }
    ...
    private fun openPoint(o: RideOffer, pickup: Boolean) {
        ...
        val uri = if (lat != null && lng != null) {
            Uri.parse("geo:$lat,$lng?q=$lat,$lng(${Uri.encode(label ?: "Sr. Rotas")})")
        } else label?.let { Uri.parse("geo:0,0?q=${Uri.encode(it)}") }
    ...
    private fun platform(o: RideOffer) = when (o.platform.lowercase()) {
        "uber" -> "Uber"
        "99" -> "99"
        else -> o.platform.ifBlank { "Oferta" }
    }
