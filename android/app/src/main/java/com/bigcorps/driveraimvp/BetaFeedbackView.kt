...
class BetaFeedbackView(context: Context) : LinearLayout(context) {
    companion object {
        private const val PREFS = "sr_rotas_beta_checklist"
        private val CHECKS = listOf(
            "Conta e onboarding",
            "Permissão do HUD",
            "Iniciar jornada / MediaProjection",
            "Ler pelo menos 10 ofertas",
            "Toque para fechar o HUD",
            "Segurar e arrastar o HUD",
    ...
    private val category = Spinner(context).apply {
        adapter = ArrayAdapter(
            ...
            listOf("Geral", "OCR", "HUD", "Conta/Login", "Histórico", "IA", "MCP", "Plano/Pix", "Notificações", "Bateria/Desempenho")
        )
    }
    private val severity = Spinner(context).apply {
        adapter = ArrayAdapter(
            context,
            android.R.layout.simple_spinner_dropdown_item,
            listOf("Sugestão", "Problema leve", "Problema importante", "Bloqueador")
        )
    }
    private val message = UiKit.input(context, "Conte o que aconteceu, o que esperava e em qual tela...", multiline = true)
    private val boxes = mutableListOf<CheckBox>()

    init {
        orientation = VERTICAL
        addView(UiKit.pill(context, "CLOSED BETA · 0.13", "primary"))
        ...
        addView(UiKit.margin(UiKit.body(context, "Checklist do teste", 13f).apply {
        ...
        CHECKS.forEachIndexed { index, label ->
            val box = CheckBox(context).apply {
                ...
                isChecked = prefs.getBoolean("check_$index", false)
                setOnCheckedChangeListener { _, checked ->
                    prefs.edit().putBoolean("check_$index", checked).apply()
                    refreshProgress()
                }
            }
            boxes += box
        ...
        addView(UiKit.margin(UiKit.body(context, "Enviar feedback", 13f).apply {
            setTypeface(typeface, Typeface.BOLD)
        }, top = 10))
        ...
        addView(UiKit.margin(UiKit.primaryButton(context, "Enviar feedback do beta") { submit() }, top = 9))
        addView(UiKit.margin(status, top = 7))
        addView(UiKit.margin(UiKit.body(context, "O relatório automático não envia OCR bruto, capturas de tela, senha, token ou chave MCP. Em caso de crash, guardamos apenas dados técnicos mínimos e o stack trace para enviar na próxima abertura.", 11f), top = 8))
    ...
    fun refresh() {
        ...
        if (pending) {
            status.text = "Há um crash técnico pendente; ele será enviado automaticamente quando a conta estiver conectada."
        } else if (status.text.isBlank()) {
            status.text = "Use este formulário sempre que encontrar algo estranho."
        }
        BetaTelemetry.flushPendingCrash(context)
    }

    private fun submit() {
        val completed = boxes.count { it.isChecked }
    ...
    private fun refreshProgress() {
        val completed = boxes.count { it.isChecked }
        progress.text = "$completed de ${CHECKS.size} itens concluídos."
    }
