...
/**
 * RC3.6 — supervisor externo ao serviço de captura.
 * Se o M1 ficar degradado por tempo sustentado, aciona o recovery já existente
 * sem finalizar a jornada nem apagar dados. M2 continua independente.
 */
                ...
                if (healthy) {
                    ...
                    if (now - unhealthySince >= UNHEALTHY_GRACE_MS && now - lastRecovery >= RECOVERY_COOLDOWN_MS) {
                        ...
                        runCatching { context.startService(intent) }
                            .onSuccess { LocalLog.append(context, "RC3.6 supervisor solicitou recuperação automática do M1 sem encerrar jornada") }
                            .onFailure { LocalLog.append(context, "RC3.6 supervisor não conseguiu chamar recovery: ${it.message}") }
                    }
                }
            }.onFailure {
                LocalLog.append(context, "RC3.6 supervisor ignorou falha isolada: ${it.message}")
            }
