 ...
 *
 * 0.27: na digitalização de jornada, após a autorização o Sr. Rotas sai da
 * frente automaticamente antes de iniciar a captura. A revisão volta ao
 * primeiro plano somente depois de o OCR concluir.
 */
class UberDigitizationActivity026 : Activity() {
    companion object {
        ...
        private const val SESSION_REVEAL_DELAY_MS = 550L
    ...
    private var pendingEmptyResult = false

    private val receiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            val text = intent?.getStringExtra(EXTRA_TEXT).orEmpty()
            if (text.isBlank()) {
                pendingEmptyResult = true
            } else {
                pendingPreviewText = UberDigitizationText0265.normalize(text)
            }

            // A Activity pode estar atrás da Uber. Trazemos a MESMA instância
            // de volta apenas quando a captura terminou.
            startActivity(
                Intent(
                    this@UberDigitizationActivity026,
                    UberDigitizationActivity026::class.java,
                ).addFlags(
    ...
    override fun onResume() {
        ...
        when {
            pendingEmptyResult -> {
                pendingEmptyResult = false
                main.post {
                    toast("Não foi possível ler a tela.")
                    finish()
                }
            }
            !pendingPreviewText.isNullOrBlank() -> {
                val text = pendingPreviewText.orEmpty()
            ...
            .setItems(
                ...
                mode =
                    if (which == 0) {
                        ...
                        UberDigitizationParser026.MODE_HISTORY
                    }
                requestCapture()
            }
    ...
    private fun requestCapture() {
        ...
        toast(
            "Autorize a captura da Uber. O Sr. Rotas mostrará a revisão antes de salvar.",
        )
    ...
    override fun onActivityResult(
        ...
        val service =
            Intent(
                this,
                UberDigitizationCaptureService026::class.java,
            )
                .putExtra(
                    UberDigitizationCaptureService026.EXTRA_RESULT_CODE,
                    resultCode,
                )
                .putExtra(
                    UberDigitizationCaptureService026.EXTRA_RESULT_DATA,
                    data,
                )
                .putExtra(EXTRA_MODE, mode)
        ...
        if (mode == UberDigitizationParser026.MODE_HISTORY) {
            ...
            toast(
                "Captura iniciada. Role o Histórico da Uber e finalize pela câmera do Sr. Rotas ou pela notificação.",
            )
            finish()
            return
        }

        // 0.27: primeiro revelamos o app que estava atrás (normalmente Uber).
        // Só então iniciamos a captura. Assim o primeiro frame não é o próprio
        // Sr. Rotas e o motorista não precisa descobrir que deve minimizá-lo.
        toast("Abrindo a Uber para capturar a jornada…")
        moveTaskToBack(true)
        main.postDelayed(
            {
                if (Build.VERSION.SDK_INT >= 26) {
    ...
    private fun previewSingle(raw: String) {
        val parsed =
            runCatching {
                UberDigitizationParser026.parse(
                    mode,
                    UberDigitizationText0265.normalize(raw),
                )
            }.getOrElse {
        ...
        when (parsed) {
            ...
            is UberDigitizationResult026.Rides ->
                previewRides(parsed.values, 1, 1)
        }
    }

    private fun previewSession(
        parsed: UberDigitizationResult026.Session,
    ) {
        val value = parsed.value
        val journey =
            value.journeyId?.let {
                LocalStore.get(this).journey(it)
            }
        val association =
            when {
                journey != null ->
                    "Jornada associada: ${dateTime(journey.startedAt)}"
                !value.journeyId.isNullOrBlank() ->
                    "Jornada associada automaticamente"
                else ->
                    "Jornada: não foi possível associar com segurança"
            }

        val message =
            buildString {
                append(
                    "Período: ${dateTime(value.startedAt)} – ${dateTime(value.endedAt)}\n",
                )
                append(
                    "Faturamento: ${value.earnings?.let(::money) ?: "—"}\n",
                )
                append(
                    "Viagens concluídas: ${value.completedTrips ?: "—"}\n",
                )
                append(
                    "Viagens oferecidas: ${value.offeredTrips ?: "—"}\n",
                )
                append("$association\n")
                append(
                    "Confiança: ${(value.confidence * 100).toInt()}%",
                )
    ...
    private fun reviewHistoryScan(fileName: String) {
        ...
        if (
            !safeName.startsWith("uber-history-scan-") ||
            ...
            runCatching {
                ...
                if (array == null) {
                    ...
                    (0 until array.length())
                        .map {
                            UberDigitizationText0265.normalize(
                                array.optString(it),
                            )
                        }
        ...
        toast(
            "Captura concluída · ${frames.size} quadro(s). Organizando os registros…",
        )
        val found =
            runCatching {
                UberHistoryScanAccumulator0262.parseFrames(frames)
            }.getOrElse {
    ...
    private fun previewRides(
        ...
        AlertDialog.Builder(this)
            ...
            .setMessage(
                "Foram analisados $framesRead quadro(s); $framesWithRides continham cartões reconhecíveis. Desmarque qualquer registro que não queira salvar.",
            )
            .setMultiChoiceItems(
                labels,
                checked,
    ...
    private fun rideLabel(
        value: UberCompletedRide026,
    ): String =
        buildString {
            ...
            append(
                if (
                    ...
                    serviceLabel(value.serviceType)
                },
            )
            append(" · ${money(value.fare)}")
            value.durationSeconds?.let {
                append(" · ${duration(it)}")
            }
            value.distanceKm?.let {
                append(" · ${fmt(it)} km")
            }
            value.surgeAmount?.let {
                append(" · din. ${money(it)}")
            }
            value.extraAmount?.let {
                append(" · extra ${money(it)}")
            }
            if (
                !value.pickupLabel.isNullOrBlank() ||
                !value.destinationLabel.isNullOrBlank()
            ) {
                append(
    ...
    private fun save(parsed: UberDigitizationResult026) {
        ...
        val message =
            when (parsed) {
                is UberDigitizationResult026.Session -> {
                    if (store.saveSession(parsed.value)) {
                        "Jornada digitalizada salva/atualizada."
                    } else {
        ...
        UberDigitizationClient026.sync(this, parsed) { result ->
            if (
                parsed is UberDigitizationResult026.Session &&
                result.isSuccess
            ) {
                JourneyRealizedClient0262.refreshDays(
                    this,
                    30,
                ) { }
            }
