...
class StrategyActivity : Activity() {
    ...
    private val metricLabels=linkedMapOf("per_hour" to "R$/hora","rating" to "Avaliação","per_minute" to "R$/min","per_km" to "R$/km","profit_hour" to "Lucro/hora","profit_percent" to "Margem %","profit" to "Lucro líquido")
    ...
    private fun buildUi():View{
        ...
        val min=thresholdCard("R$/min","Valor da oferta dividido pelos minutos totais.",0.48,0.60);redMinute=min.first;greenMinute=min.second;root.addView(min.third)
        ...
        root.addView(space(18));root.addView(section("Aparência do HUD"));root.addView(body("Escolha o que aparece no card flutuante e use ↑/↓ para definir a ordem."));metricsBox=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL};root.addView(metricsBox)
        ...
        root.addView(label("Tamanho da fonte (11–20)"));fontSize=SeekBar(this).apply{max=9};root.addView(fontSize)
