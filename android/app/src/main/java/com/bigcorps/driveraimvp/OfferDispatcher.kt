...
class OfferDispatcher(context: Context) {
    ...
    fun dispatch(offer: RideOffer, showOverlay: Boolean = true): Boolean {
        ...
        LocalLog.append(appContext,"OFERTA ${enriched.offerType} confiança=${enriched.confidence}: ${summary.replace('\n',' ')}")
        if(showOverlay) overlay.show(enriched)
        BackendClient.sendOffer(appContext,enriched)
        broadcast(); return true
    }

    fun dispatchAll(offers: List<RideOffer>) {
        val emitted=offers.filter { dispatch(it,false) }; if(emitted.isEmpty()) return
        emitted.maxWithOrNull(compareBy<RideOffer> { verdictRank(it.verdict) }.thenBy { it.confidence }.thenBy { it.perKm ?: 0.0 })?.let(overlay::show)
    }
