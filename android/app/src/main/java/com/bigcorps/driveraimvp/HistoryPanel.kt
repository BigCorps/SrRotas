...
class HistoryPanel(context: Context) : LinearLayout(context) {
    ...
    private val serviceSpinner = spinner(listOf("Todos serviços", "UberX", "Comfort", "Black", "Electric", "Priority", "Moto", "Desconhecido"))
    ...
    init {
        ...
        addView(UiKit.card(context).apply {
    ...
    private fun topOffersCard(data: HistoryAnalytics): View = UiKit.card(context).apply {
        ...
        data.topOffers.take(6).forEach { o ->
            ...
            addView(LinearLayout(context).apply {
                orientation = HORIZONTAL; gravity = Gravity.CENTER_VERTICAL; setPadding(0, UiKit.dp(context, 7), 0, UiKit.dp(context, 7))
    ...
    private fun journeysCard(data: HistoryAnalytics): View = UiKit.card(context).apply {
        ...
        data.journeys.take(12).forEach { j ->
            addView(LinearLayout(context).apply {
                orientation = VERTICAL; setPadding(0, UiKit.dp(context, 7), 0, UiKit.dp(context, 7))
