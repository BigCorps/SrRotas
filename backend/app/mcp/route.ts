...
const allowedHeaders = ["Authorization","Content-Type","Accept","MCP-Protocol-Version","MCP-Session-Id","Last-Event-ID"].join(", ");
...
export async function OPTIONS() {
  return new Response(null, { status: 204, headers: {
    ...
    "Access-Control-Allow-Headers":allowedHeaders, "Access-Control-Max-Age":"86400",
  }});
}
...
async function handle(request: Request) {
  ...
  if (!context) {
    return withCors(new Response(JSON.stringify({ error:"unauthorized" }), { status:401, headers:{
      "content-type":"application/json", "WWW-Authenticate":'Bearer realm="sr-rotas-mcp"',
    }}));
  }
