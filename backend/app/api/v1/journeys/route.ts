...
export async function POST(request: Request) {
  // Escrita de jornada continua exclusiva do Android/device token.
  ...
  try {
    ...
    if (action === "start") {
      return Response.json({
        ok: true,
        journey: await startJourney(auth.driverId, auth.deviceId, body as Record<string, unknown>),
      });
    }
...
export async function GET(request: Request) {
  // Leitura pode ser feita pelo Android ou pela sessão Web temporária.
  ...
  try {
    ...
    if (id) {
      ...
      if (url.searchParams.get("include_offers") === "1") {
        ...
        return Response.json({
          ...detail,
          offers: found.offers,
          offer_count_returned: found.offers.length,
        });
      }
    ...
    if (url.searchParams.get("current") === "1") {
      return Response.json({ journey: await currentJourney(auth.driverId) });
    }

    return Response.json({
      journeys: await listJourneys(auth.driverId, Number(url.searchParams.get("limit") || 30)),
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : "journey_failed";
    return Response.json({ error: message }, { status: message === "journey_not_found" ? 404 : 500 });
